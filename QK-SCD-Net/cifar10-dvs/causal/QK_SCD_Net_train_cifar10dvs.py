...
def get_annealed_lambdas(epoch, max_epochs, warmup_epochs=15, anneal_epochs=45):
    """
    针对 DVS 较短 epoch (96) 调整退火策略
    """
    max_lambda_c = 1.0
    max_lambda_s = 0.0
...
def parse_args():
    ...
    parser.add_argument('--data-path', default='D:/dataset/data/cifar10dvs/', help='dataset')
    ...
    parser.add_argument('--epochs', type=int, default=96, metavar='N', help='number of epochs to train')
    ...
    parser.add_argument('--warmup-lr', type=float, default=1e-5, metavar='LR', help='warmup learning rate')
    ...
    parser.add_argument('--mixup-mode', type=str, default='batch',
                        help='mixup/cutmix params per "batch", "pair", or "elem"')
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, print_freq, scaler=None, T_train=None,
    ...
    for batch_idx, (image, target) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if scaler is not None:
            ...
            output_full, output_masked, mask_prob, firing_num_t, firing_rate = model(image, epoch)
...
def main(args):
    ...
    # 设定根目录为 ./output，生成本次运行的主文件夹路径
    base_output_dir = "./output/train_causal"
