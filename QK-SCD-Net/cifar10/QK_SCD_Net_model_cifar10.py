...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        # ==== Stage 2: Depthwise Conv (新增) ====
        # 保持原有 S_MLP 的 LIF -> Conv -> BN 范式
        x = self.dw_lif(x)
...
class TokenSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.tssa = Token_QK_Attention(dim, num_heads)   #调用SOP_QKTA
...
class SpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.ssa = Spiking_Self_Attention(dim, num_heads)   #调用SOP_SSA
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = MLP(in_features= dim, hidden_features=mlp_hidden_dim, drop=drop)
...
class spiking_transformer(nn.Module):
    def __init__(self,
        ...
        stage1 = nn.ModuleList([TokenSpikingTransformer(   #调用QKTA
        ...
        stage2 = nn.ModuleList([TokenSpikingTransformer(   #调用QKTA
