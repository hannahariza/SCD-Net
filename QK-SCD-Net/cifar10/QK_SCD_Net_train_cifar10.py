...
def move_train_log_to_output_dir(logger: logging.Logger, output_dir: str):
    """把当前 train logger 的 FileHandler 文件搬到 output_dir，不创建第二个文件。"""
    ...
    file_handler.close()
    try:
        os.replace(old_path, new_path)  # 原子替换/移动
...
def train_one_epoch(
        epoch, num_epochs, start_time, model, loader, optimizer, loss_fn, args,
    ...
    lambda_c, lambda_s = get_annealed_lambdas(epoch, args.epochs)
    ...
    return OrderedDict([('loss_total', losses_m.avg)])


def validate(start_time, model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='', epoch=None):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    loss_masked_m = AverageMeter()
    loss_causal_m = AverageMeter()
            ...
            if args.local_rank == 0 and (last_batch or (batch_idx + 1) % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    'Acc@5: {top5.val:>7.4f} ({top5.avg:>7.4f})'.format(
                        log_name, et, batch_idx, last_idx,
...
def main():
    ...
    # 应用临时目录创建逻辑
    global _logger
    _logger, tmp_log_path = setup_train_logger_with_temp("./output/train_logs")
    _logger.info(f"Logging to file: {tmp_log_path}")
    ...
    saver = None
    output_dir = None
    start_time = time.time()  # 记录全局启动时间，用于 et 计算
    ...
    if args.rank == 0:
        ...
        output_dir = get_outdir(args.output if args.output else './output/train_causal', exp_name)

        # 将日志从临时目录安全迁移至最终输出目录
        move_train_log_to_output_dir(_logger, output_dir)
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            # 传入 num_epochs 和 start_time
            train_metrics = train_one_epoch(
                epoch, num_epochs, start_time, model, loader_train, optimizer, train_loss_fn, args,
            ...
            # 传入 start_time
            eval_metrics = validate(start_time, model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast,
                                    epoch=epoch)
            ...
            if model_ema is not None and not args.model_ema_force_cpu:
                ...
                ema_eval_metrics = validate(
                    start_time, model_ema.module, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast,
                    epoch=epoch,
                    log_suffix=' (EMA)')
