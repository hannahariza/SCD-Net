...
import model_causal  # 引用包含了 QKFormer_causal 的 model.py
...
parser.add_argument('-data-dir', metavar='DIR', default="/media/data/spike-transformer-network/torch/cifar100/",
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast(enabled=True):
            ...
            output_full = fuse_logits_by_firing(output_full, fr_full, eps=1e-6)
...
def main():
    ...
    # 修改为使用你在 model.py 中新定义的带有 causal 功能的类 QKFormer_causal
    model = create_model(
        "QKFormer_causal",
        pretrained=False,
        drop_rate=0.,
    ...
    if args.rank == 0:
        ...
        # 将输出文件夹路径变更为专门针对 CIFAR-100 的标识
        output_dir = get_outdir(args.output if args.output else './output/train_cifar100_causal', exp_name)
