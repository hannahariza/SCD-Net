...
try:
    import qkformer_fixed_baseline as qkformer
except ImportError:
    import qkformer
...
def get_args_parser():
    ...
    parser.add_argument('--lambda_intervened_ce', type=float, default=1.0)
    parser.add_argument('--lambda_kl', type=float, default=1.0)
    parser.add_argument('--lambda_sparse', type=float, default=5e-4)
    parser.add_argument('--causal_kd_temp', type=float, default=3.0)
...
def save_checkpoint_fixed(args, model_without_ddp, optimizer, loss_scaler, epoch, filename, max_accuracy=None, current_accuracy=None, model_ema=None, max_ema_accuracy=None, current_ema_accuracy=None):
    ...
    if max_ema_accuracy is not None:
        to_save['max_ema_accuracy'] = float(max_ema_accuracy)
    if current_ema_accuracy is not None:
        to_save['current_ema_accuracy'] = float(current_ema_accuracy)
...
def main(args):
    ...
    if model_ema is not None and args.resume:
        resume_ckpt = torch.load(args.resume, map_location='cpu') if not args.resume.startswith('https') else None
        if resume_ckpt is not None and 'model_ema' in resume_ckpt:
            model_ema.module.load_state_dict(resume_ckpt['model_ema'])
            print('Resume EMA checkpoint %s' % args.resume)

    if args.eval:
        test_stats = evaluate(data_loader_val, model, device, args=args, epoch=args.start_epoch)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        ema_test_stats = None
        current_ema_accuracy = None
        if model_ema is not None:
            ema_test_stats = evaluate(data_loader_val, model_ema.module, device, args=args, epoch=epoch)
            current_ema_accuracy = float(ema_test_stats['acc1'])
            print(f"EMA accuracy of the network on the {len(dataset_val)} test images: {current_ema_accuracy:.1f}%")
        ...
        save_checkpoint_fixed(
            ...
            model_ema=model_ema,
            current_ema_accuracy=current_ema_accuracy,
        )
        ...
        if is_best:
            ...
            save_checkpoint_fixed(
                ...
                current_accuracy=current_accuracy,
                model_ema=model_ema,
                max_ema_accuracy=max_ema_accuracy,
                current_ema_accuracy=current_ema_accuracy,
            )

        if current_ema_accuracy is not None:
            max_ema_accuracy = max(max_ema_accuracy, current_ema_accuracy)

        cleanup_old_checkpoints(args.output_dir)
        print(f'Max accuracy: {max_accuracy:.2f}%')
        if current_ema_accuracy is not None:
            print(f'Max EMA accuracy: {max_ema_accuracy:.2f}%')
        ...
        log_stats = {
            ...
            **({f'ema_test_{k}': v for k, v in ema_test_stats.items()} if ema_test_stats is not None else {}),
            'epoch': epoch,
