...
class STEBinary(torch.autograd.Function):
    ...
    def forward(ctx, input):
        # Generate 0/1 binary matrix to maintain low-energy SNN characteristics
...
class DynamicCausalMask(nn.Module):
    ...
    def forward(self, x):
        ...
        # Priority to 0/1 binary matrices
        mask = STEBinary.apply(prob)
        mask_expanded = mask.unsqueeze(0).expand(T, B, C, H, W)
        out_masked = x * mask_expanded
        return out_masked, mask
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.):
        ...
        # DWConv added from mixer_hub structure
        self.dw_conv = nn.Conv2d(
            hidden_features, hidden_features, kernel_size=3, padding=1, groups=hidden_features, bias=False
        )
    ...
    def forward(self, x):
        ...
        x = self.dw_bn(x).reshape(T, B, self.c_hidden, W, H).contiguous()
        x = self.dw_lif(x)
...
class hierarchical_spiking_transformer(nn.Module):
    ...
    def forward_features(self, x, enable_causal=True):
        ...
        # 掩码干预生成
        x_masked, mask = self.causal_mask(x)
        ...
        return feat_full, feat_masked, mask
    ...
    def forward(self, x, epoch=None, enable_causal=True):
        ...
        feat_full, feat_masked, mask = self.forward_features(x)

        firing_num_full_t = feat_full.sum(dim=2)
        out_full = self.head(feat_full)

        # 掩码干预分支
        firing_num_masked_t = feat_masked.sum(dim=2)
        # 将原有发射率指标替换为 0/1 掩码矩阵通过率 (Pass-through rate)
        pass_through_rate = mask.mean(dim=(1, 2, 3)).unsqueeze(0).expand(T, -1)
        out_masked = self.head(feat_masked)

        return out_full, out_masked, mask, firing_num_full_t, firing_num_masked_t, pass_through_rate
