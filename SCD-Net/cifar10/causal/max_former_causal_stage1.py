...
class Max_Former(nn.Module):
    def __init__(self, in_channels=2, num_classes=11,
        ...
        # 加入因果掩码干预模块：在 patch_embed1 之后、stage1 之前，通道维度为 embed_dims // 4
        self.causal_mask = DynamicCausalMask(in_channels=embed_dims // 4)
    ...
    def forward_features(self, x):
        ...
        x_full_deep = patch_embed2(x_full_deep)
        for blk in stage2:
            x_full_deep = blk(x_full_deep)
        x_full_deep = patch_embed3(x_full_deep)
        for blk in stage3:
            x_full_deep = blk(x_full_deep)
        ...
        # 重置下游状态，避免完整分支的信息泄露到干预分支
        functional.reset_net(stage1)
        ...
        # 2. 掩码干预分支 (Student)
        x_masked_deep = x_masked
        for blk in stage1:
            x_masked_deep = blk(x_masked_deep)
        x_masked_deep = patch_embed2(x_masked_deep)
        for blk in stage2:
            x_masked_deep = blk(x_masked_deep)
        x_masked_deep = patch_embed3(x_masked_deep)
        for blk in stage3:
            x_masked_deep = blk(x_masked_deep)
