...
import max_former_causal_after_stage3
import ms_qkformer
...
# ================= 新增因果损失相关辅助函数 =================
def get_annealed_lambdas(epoch, max_epochs, warmup_epochs=30, anneal_epochs=150):
    """
    退火策略，动态计算因果损失(lambda_c)和稀疏损失(lambda_s)的权重
    """
    max_lambda_c = 5.0
    max_lambda_s = 5e-4
    ...
    if epoch < warmup_epochs:
        return 0.0, 0.0
    elif epoch < warmup_epochs + anneal_epochs:  # 线性增强
...
def fuse_logits_by_firing(output, firing_num_t, eps=1e-6):
    ...
    assert firing_num_t.dim() == 2, f"Expect firing_num_t dim=1 or 2, got {firing_num_t.dim()}"
    assert firing_num_t.shape[0] == T and firing_num_t.shape[1] == N, \
        f"Shape mismatch: output {output.shape}, firing_num_t {firing_num_t.shape}"
...
def main():
    ...
    _logger.info(f"Training session started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    # ===============================================================
    ...
    if args.local_rank == 0:
        ...
        output_dir = get_outdir(args.output if args.output else './output/train_10_causal', exp_name)

        # ================= 日志文件重定向 =================
        log_file = os.path.join(output_dir, f'train_{exp_name}.log')
        file_handler = logging.FileHandler(log_file)
        file_handler.setFormatter(logging.Formatter("%(asctime)s [%(levelname)s] %(message)s", "%Y-%m-%d %H:%M:%S"))
        _logger.addHandler(file_handler)
        _logger.info(f"Logging configured. Output will be saved to: {log_file}")
        # ========================================================

        # ================= 打印并记录模型摘要 =================
        if has_summary:
            _logger.info("Generating Model Summary...")
            try:
                input_shape = (1, 3) + data_config['input_size'][1:]
                try:
                    stats = summary(model, input_size=input_shape, epoch=0, verbose=0)
                except TypeError:
                    stats = summary(model, input_size=input_shape, verbose=0)
                _logger.info(f"\n{str(stats)}")
            except Exception as e:
                _logger.warning(f"Could not generate model summary due to: {e}")
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            # 传入 num_epochs 供退火策略使用
            train_metrics = train_one_epoch(
                epoch, num_epochs, model, loader_train, optimizer, train_loss_fn, args,
            ...
            if args.local_rank == 0:
                ...
                total_time_str = get_elapsed_time(start_absolute_time)

                # ================= 精细化 Epoch 总结打印 =================
                _logger.info(
                    f"\n{'=' * 50}\n"
                    f"-> Epoch {epoch} Summary | Time: {epoch_time:.2f}s | Total Running Time: {total_time_str}\n"
                    f"   [Train] Acc@1: {train_metrics['top1']:.4f} | Acc@5: {train_metrics['top5']:.4f}\n"
                    f"   [Val]   Acc@1: {eval_metrics['top1']:.4f} | Acc@5: {eval_metrics['top5']:.4f}\n"
                    f"   Current Max Val Acc: {max_accuracy:.4f}\n"
                    f"{'=' * 50}"
                )
                # =========================================================
                ...
                if args.log_wandb and has_wandb:
                    wandb.log({
                        ...
                        "train/acc_tp1": train_metrics['top1'],
                        "val/loss_total": eval_metrics['loss'],
    ...
    _logger.info(f"Total Training Duration: {get_elapsed_time(start_absolute_time)}")


def train_one_epoch(
        epoch, num_epochs, model, loader, optimizer, loss_fn, args,
    ...
    loss_masked_m = AverageMeter()
    loss_causal_m = AverageMeter()
    loss_full_m = AverageMeter()
    loss_sparse_m = AverageMeter()
    firing_rates_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
    # ====================================================
    ...
    # 稀疏损失消融：只保留 KL 损失的退火权重
    lambda_c, _ = get_annealed_lambdas(epoch, num_epochs)
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            # 修改获取 6 个输出
            output_full, output_masked, mask_prob, fr_full, fr_masked, firing_rate = model(input, epoch)
            ...
            firing_rates_m.update(step_firing_rates.detach().cpu().numpy(), input.size(0))

            output_full = fuse_logits_by_firing(output_full, None, eps=1e-6)
            output_masked = fuse_logits_by_firing(output_masked, None, eps=1e-6)

            # 1. 分类损失
            loss_full = loss_fn(output_full, target)
            loss_masked = loss_fn(output_masked, target)

            loss_causal = torch.tensor(0.0, device=input.device)
            loss_sparse = torch.tensor(0.0, device=input.device)
            ...
            # 3. 稀疏性损失消融：不计算、不加入总损失

            # 4. 总损失合并：full CE + mask CE + KL
            loss_total = loss_full + loss_masked + lambda_c * loss_causal
        ...
        acc1, acc5 = accuracy(output_masked, target_for_acc, topk=(1, 5))

        if not args.distributed:
            losses_m.update(loss_total.item(), input.size(0))
            loss_masked_m.update(loss_masked.item(), input.size(0))
            loss_causal_m.update(loss_causal.item(), input.size(0))
            loss_full_m.update(loss_full.item(), input.size(0))
            loss_sparse_m.update(loss_sparse.item(), input.size(0))
        ...
        if loss_scaler is not None:
            loss_scaler(
                loss_total, optimizer,
                clip_grad=args.clip_grad, clip_mode=args.clip_mode,
                parameters=model_parameters(model, exclude_head='agc' in args.clip_mode),
                create_graph=second_order)
        else:
            loss_total.backward(create_graph=second_order)
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            # 放电率字符串转换
            fr_avg = firing_rates_m.avg
            if isinstance(fr_avg, np.ndarray):
                fr_str = "[" + ", ".join([f"{x:.4f}" for x in fr_avg]) + "]"
            else:
                fr_str = f"{fr_avg:.4f}"
            ...
            if args.distributed:
                reduced_loss = reduce_tensor(loss_total.data, args.world_size)
                losses_m.update(reduced_loss.item(), input.size(0))

                reduced_acc1 = reduce_tensor(acc1, args.world_size)
                reduced_acc5 = reduce_tensor(acc5, args.world_size)
                top1_m.update(reduced_acc1.item(), input.size(0))
                top5_m.update(reduced_acc5.item(), input.size(0))

            if args.local_rank == 0:
                # ================= 更新日志打印格式，包含四大损失和 FR =================
                _logger.info(
                    'Train: {} [{:>4d}/{} ({:>3.0f}%)]  '
                    'L_Tot: {loss_total.avg:>6.4f}  '
                    'L_Full: {loss_full.avg:>6.4f}  '
                    'L_Mask: {loss_masked.avg:>6.4f}  '
                    'L_Caus: {loss_causal.avg:>6.4f}  '
                    'L_Spar: {loss_sparse.avg:>6.4f}  '
                    'FR: {fr_str}  '
                    ...
                    'LR: {lr:.3e}'.format(
                        ...
                        loss_total=losses_m,
                        loss_full=loss_full_m,
                        loss_masked=loss_masked_m,
                        loss_causal=loss_causal_m,
                        loss_sparse=loss_sparse_m,
                        fr_str=fr_str,
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix='', epoch=None):
    batch_time_m = AverageMeter()
    ...
    total_epochs = getattr(args, 'epochs', 400)
    current_epoch = epoch if epoch is not None else total_epochs
    # 稀疏损失消融：只保留 KL 损失的退火权重
    lambda_c, _ = get_annealed_lambdas(current_epoch, total_epochs)
            ...
            if args.local_rank == 0 and (last_batch or batch_idx % args.log_interval == 0):
                ...
                _logger.info(
                    ...
                    'Acc@5: {top5.val:>7.4f} ({top5.avg:>7.4f})'.format(
                        ...
                        loss_total=losses_m, loss_causal=loss_causal_m, loss_sparse=loss_sparse_m,
