...
def main():
    ...
    if args.local_rank == 0:
        ...
        output_dir = get_outdir(args.output if args.output else './output/train_100_causal', exp_name)
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if args.local_rank == 0:
                ...
                _logger.info(
                    ...
                    f"Total Running Time: {total_time_str} | "
                    f"Current Max Acc: {max_accuracy:.4f}"
                )
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            # 修改获取 5 个输出
            output_full, output_masked, mask_prob, firing_num_t, firing_rate = model(input, epoch)
            ...
            output_full = fuse_logits_by_firing(output_full, firing_num_t, eps=1e-6)
            output_masked = fuse_logits_by_firing(output_masked, firing_num_t, eps=1e-6)
