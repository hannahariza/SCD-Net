...
from model.maxformer_causal_imagenet100.utils import *
from model.maxformer_causal_imagenet100.utils import infer_num_classes, is_tiny_imagenet_layout, normalize_dataset_name
from model.maxformer_causal_imagenet100.utils import train_one_epoch, evaluate
import model.maxformer_causal_imagenet100.misc as misc
from model.maxformer_causal_imagenet100.misc import NativeScalerWithGradNormCount as NativeScaler
...
from model.maxformer_causal_imagenet100.max_former import maxformer
from model.maxformer_causal_imagenet100.ms_qkformer import ms_qkformer
# import wandb
...
def get_args_parser():
    ...
    parser.add_argument('--dataset', default='imagenet100', type=str,
                        help='dataset name: imagenet100 or imagenet1k')
    ...
    if args.data_path and is_tiny_imagenet_layout(args.data_path):
        args.dataset = 'tinyimagenet'
    else:
        args.dataset = normalize_dataset_name(args.dataset)

    inferred_nb_classes = infer_num_classes(args.dataset)
    if args.nb_classes != inferred_nb_classes:
        print(f'Overriding nb_classes from {args.nb_classes} to {inferred_nb_classes} for dataset {args.dataset}')
        args.nb_classes = inferred_nb_classes

    if args.output_dir and not args.log_dir:
...
def main(args):
    model.maxformer_causal_imagenet100.misc.init_distributed_mode(args)
    ...
    if args.distributed:
        num_tasks = model.maxformer_causal_imagenet100.misc.get_world_size()
        global_rank = model.maxformer_causal_imagenet100.misc.get_rank()
        ...
        global_rank = 0  # <--- 新增这一行，指定单卡模式的 rank 为 0
    ...
    model = create_model(args.model, T=args.time_step, num_classes=args.nb_classes)
    ...
    eff_batch_size = args.batch_size * args.accum_iter * model.maxformer_causal_imagenet100.misc.get_world_size()
    ...
    model.maxformer_causal_imagenet100.misc.load_model(args=args, model_without_ddp=model_without_ddp, optimizer=optimizer, loss_scaler=loss_scaler)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and model.maxformer_causal_imagenet100.misc.is_main_process():
            update_summary(
                epoch,
                train_stats,
                test_stats,
